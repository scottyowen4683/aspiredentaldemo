-- Migration: Update billing model to flat rate + interaction tracking + overage
-- Interactions = SMS, phone call (inbound/outbound), or chat session

-- Update organizations table with new billing structure
DO $$
BEGIN
  -- Remove old per-interaction pricing
  ALTER TABLE organizations DROP COLUMN IF EXISTS monthly_interaction_limit;
  ALTER TABLE organizations DROP COLUMN IF EXISTS price_per_interaction;

  -- Add new flat rate billing columns
  ALTER TABLE organizations ADD COLUMN IF NOT EXISTS flat_rate_fee DECIMAL(10,2) DEFAULT 0.00;
  ALTER TABLE organizations ADD COLUMN IF NOT EXISTS included_interactions INTEGER DEFAULT 5000;
  ALTER TABLE organizations ADD COLUMN IF NOT EXISTS overage_rate_per_1000 DECIMAL(10,2) DEFAULT 0.00;
...
CREATE TABLE IF NOT EXISTS interaction_logs (
  ...
  -- Contact info
  contact_number TEXT, -- Phone number or identifier

  -- Metadata
  duration_seconds INTEGER, -- For calls
  message_count INTEGER, -- For chat sessions
  cost DECIMAL(10,6) DEFAULT 0, -- Actual API cost
...
CREATE TABLE IF NOT EXISTS outbound_campaigns (
  ...
  call_hours_start TIME DEFAULT '09:00:00', -- 9 AM
  call_hours_end TIME DEFAULT '17:00:00', -- 5 PM
...
CREATE TABLE IF NOT EXISTS campaign_contacts (
  ...
  last_name TEXT,
  email TEXT,
  custom_fields JSONB, -- Additional fields from CSV
...
SELECT
  ...
  -- Usage percentage
  ROUND((o.current_period_interactions::DECIMAL / o.included_interactions * 100), 2) AS usage_percentage
