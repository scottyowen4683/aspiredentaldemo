 ...
 *  - Read session + transcript (from turns; fallback session_transcripts)
 *  - Call OpenAI (EVAL_MODEL) -> {overall:number, summary:string}
...
/** Build a readable transcript from `turns`; fallback to `session_transcripts` if empty */
async function getTranscript(session_id) {
  // Prefer turns (the webhook writes them)
  const turnsRes = await sbFetch(
    `/rest/v1/turns?session_id=eq.${session_id}&select=role,content,created_at&order=created_at.asc`
  );
  if (turnsRes.ok) {
    const turns = await turnsRes.json();
    if (Array.isArray(turns) && turns.length) {
      const lines = turns.map(t => {
        const role = String(t.role || "").toLowerCase() === "user" ? "User" : "AI";
        return `${role}: ${t.content || ""}`.trim();
      });
      return lines.join("\n");
    }
  }

  // Fallback to session_transcripts if present
  const trRes = await sbFetch(`/rest/v1/session_transcripts?session_id=eq.${session_id}&select=content&limit=1`);
  if (trRes.ok) {
...
function buildEvalPrompt({ transcript, sessionSummary }) {
  const rubric = `
You are an evaluator for phone-call assistants. Score the assistant's performance from 0–100 and write a concise 1–2 sentence overview.
Consider: clarity, helpfulness, tone, accuracy, and whether the caller's purpose was addressed.
Return STRICT JSON ONLY exactly in this shape:
{"overall": <number 0-100>, "summary": "<short text>"}.
`;
  return `${rubric}
...
exports.handler = async (event) => {
  try {
    ...
    for (const r of runs) await processOne(r);
